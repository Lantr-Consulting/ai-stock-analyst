...
def _all_demo_users(*, clean_expired: bool = False) -> list[dict[str, Any]]:
    body = _auth_request(
        "GET", "admin/users", params={"page": 1, "per_page": 1000}, expected=(200,)
    )
    ...
    for auth_user in users:
        ...
        if clean_expired and expires and expires <= now:
            try:
                _auth_request(
                    "DELETE", f"admin/users/{auth_user['id']}", expected=(200, 204)
                )
...
def create_session(
    ...
    *,
    ...
    meta = {
        "demo_kind": DEMO_KIND,
        "demo_language": "en" if language == "en" else "zh",
...
def status(user: dict[str, Any]) -> dict[str, Any]:
    if not is_demo_user(user):
        return {"isDemo": False}
    auth_user = _auth_request("GET", f"admin/users/{user['id']}", expected=(200,))
    meta = _metadata(auth_user)
    limit = int(meta.get("ai_actions_limit", _action_limit()))
    used = int(meta.get("ai_actions_used", 0))
    return {
        "isDemo": True,
        "expiresAt": meta.get("demo_expires_at"),
        "aiActionsUsed": used,
        "aiActionLimit": limit,
...
def consume_ai_action(user: dict[str, Any]) -> None:
    """Charge one live-AI operation to a guest. Normal accounts are untouched."""
    if not is_demo_user(user):
        return
    with _quota_lock:
        auth_user = _auth_request("GET", f"admin/users/{user['id']}", expected=(200,))
        ...
        daily_used = sum(
            int(_metadata(item).get("ai_actions_used", 0))
...
def is_demo_user_id(user_id: str) -> bool:
    try:
        auth_user = _auth_request("GET", f"admin/users/{user_id}", expected=(200,))
    except HTTPException:
        return False
